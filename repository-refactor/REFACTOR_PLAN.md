...
Status as of 2026-07-24: **research complete through ten review rounds;
implementation has begun.** Phases 1 through 4 are merged and verified:
settings behavior and composer geometry/character-cap behavior are
characterized and extracted. Unstarted phases remain proposals until work
begins on their bounded phase branch.
...
cutover is not possible. Phases 1 through 4 are complete. The remaining
phase rows stay proposals until started, and the combined result receives a
...
Phases 1 through 4 are complete. Unstarted rows remain proposals that Scott may
...
| 3 | **Completed 2026-07-24** — Composer geometry characterization (test-only), [PR #18](https://github.com/scottonanski/ChickenButt/pull/18), merge `3c0c996a8d7366b32f5a30d38a5abf1b05bc6e14`. Actual scope: added 42 assertions to `scripts/test_sidebar_interactions.py`; no production-code changes. The tests cover all eight Phase 4 methods, sizing and scrollbar transitions, line/content/window fallbacks, char counting and both truncation paths, placeholder behavior, direct/idle alignment callbacks, surface-hook retry/idempotence/immediate/layout behavior, and real initial/`changed`/`insert-text`/`realize`/`map` wiring. Verification: Python compilation and diff checks passed; targeted script 101/0; all 15 documented scripts passed in both implementation and review runs, including Meson-backed 48/0 and 37/0 checks with no skips; exact-head [PR CI run 30070871601](https://github.com/scottonanski/ChickenButt/actions/runs/30070871601) and synchronized-`main` [CI run 30071149550](https://github.com/scottonanski/ChickenButt/actions/runs/30071149550) passed. Preserved guarantee: current composer sizing, fallbacks, truncation, presentation, alignment, and construction-signal behavior are locked before extraction. Its Phase 4 dependency and documented live resize check are complete. No scope deviation; the existing test script was extended so the documented suite remains 15 scripts. | — | n/a | Completed |
| 4 | **Completed 2026-07-24** — Composer geometry/character-cap extraction, [PR #19](https://github.com/scottonanski/ChickenButt/pull/19), merge `2651f7f34953446a00eeeeab86551775153f963f`. Actual scope: moved the eight characterized methods and six composer constants into new runtime module `composer_geometry.py`; `ComposerGeometry` now owns `_composer_truncating`/`_composer_layout_hooked` and receives the input, scroller, placeholder, character label, alignment callback, narrow surface/height/default-size providers, and fallback window height explicitly. `ChatSidebar` retains alignment, hint, and input-key behavior, re-exports the existing composer constants, and wires initial sizing plus buffer `changed`/`insert-text` and window `realize`/`map` callbacks to the extracted owner. Meson and installed-layout coverage include the new module; the Phase 3 characterization targets the new owner and adds a private-state ownership assertion. Verification: Python compilation and diff checks passed; targeted script 102/0; all 15 documented scripts passed in one staged-tree run, including Meson-backed 49/0 and 37/0 checks with no skips; a live presented-window resize check verified the eight-line 720px cap, six-line 500px cap, overflow scrolling/bottom alignment, and short-text no-scroll/center alignment; exact-head [PR CI run 30072168724](https://github.com/scottonanski/ChickenButt/actions/runs/30072168724) and synchronized-`main` [CI run 30072443273](https://github.com/scottonanski/ChickenButt/actions/runs/30072443273) passed. Preserved guarantee: composer sizing, fallbacks, truncation, placeholder/counter behavior, scrollbar transitions, action alignment, surface-layout handling, and construction-signal behavior remain unchanged. No remaining Phase 4 compatibility facade or scope deviation. | Phase 3 | Low | Completed |
