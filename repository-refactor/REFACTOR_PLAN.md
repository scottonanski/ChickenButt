...
Status as of 2026-07-23: **research complete, through nine rounds of
...
The phase plan below has gone through nine review rounds. Rounds 1-7
...
`test_desktop_integration.py` 37/37, zero skips); rounds 2-9 were
...
notes for the full findings list of all nine rounds.
...
**What's actually agreed right now:** the findings from all nine rounds,
...
**What's not yet locked:** the precise phase table is now on its ninth
...
## Proposed phase sequence (from `research/00-window-audit.md` §7, round 9)
...
| 24 | Message-action extraction (group L, native-rendering **and message-ID allocation excluded** — both belong elsewhere per round 7) — **migrates onto Phase 22's interface**, rebinds Phase 16's `on_intent` provider, and retires transcript action/removal delegators that no longer have callers (facade not yet removed; group M still uses it) | Phase 16, Phase 22, Phase 23 | Medium-high | Not yet |
...
| 26 | Streaming-engine extraction (group M), last — migrates onto the Phase-8 health, Phase-10 model-session, Phase-12/14/16 transcript, Phase-18 sidebar/title, Phase-20 CLI, and Phase-22 message-state interfaces; then a whole-file inventory gates removal of the last transcript/sidebar/model-session compatibility delegators and the Phase-22 messages facade. Phase-20 `is_busy()`/`try_command()` remain the permanent CLI interface | Phases 12-16, 22, 24, 25 (and in practice 8, 10, 18, 20, since `_send` calls into all of them) | High | Not yet |
...
| **Message-ID allocation** (`_next_msg_id`, `_msg_counter`) | Called from F (`_apply_restored_transcript`), K (`_post_status_message`), M (`_send`, `_start_assistant_stream`), native rendering (`_append_message`) — **never L** | **Phase 22** | Plain `ChatSidebar` method (`_next_msg_id`) unchanged until Phase 22; Phases 12/16/20 receive it as an injected provider rather than reaching back into the window | Phase 22 rebinds the replay/native-rendering/CLI providers from Phases 12/16/20; Phase 26's streaming engine consumes the owner directly | N/A (thin delegator retained, not a raw-attribute facade) | Format guarantees (`{prefix}-{counter}-{uuid suffix}`) added to Phase 21's test scope |
