...
| RR-04 | Remove `x11_sidebar.py`; remove its `meson.build` allowlist entry; update `test_installed_layout.py:64` | §4 "Dead or unreachable code" | code complete, PR open, awaiting Scott's merge | PR #7 (`recovery/rr-04-remove-x11-sidebar`) | `test_installed_layout.py` updated to positively assert absence (moved to `FORBIDDEN_TOP_LEVEL`) rather than just dropping the requirement; 13/15 test scripts run directly, 0 failures; real `meson install` still pending — meson/ninja unavailable in the environment this was executed in, no passwordless sudo to install them | Scott decided "remove" was correct in principle (§4's own framing: "definitively dead, not in progress"); Claude executed under Scott's 2026-07-23 instruction to proceed without per-item confirmation (§7) — merge is the actual approval gate |
...
| RR-07 | Delete `chickenbutt-logo.png` and the 4 orphaned `chickenbutt-panel.png` files | §4 "Dead or unreachable assets" | code complete, PR open, awaiting Scott's merge | PR #6 (`recovery/rr-07-orphaned-assets`) | grep re-confirmed zero references immediately before deletion (rule 9); 13/15 test scripts run directly, 0 failures | Scott (merge) |
...
| RR-11 | Final repository verification | all prior tasks | blocked — depends on RR-00 through RR-10 (and RR-12) each reaching verified-complete, explicitly-retained, or explicitly-deferred status (§7) | none yet | full test run recorded, real build/install/run, `main` synced with `origin` | verification only |
