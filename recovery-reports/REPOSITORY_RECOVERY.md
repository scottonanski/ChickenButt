...
| RR-05 | Remove `window.py`'s unreachable `open_link` intent handler | §4 "Dead or unreachable code" | code complete, PR open, awaiting Scott's merge | PR #8 (`recovery/rr-05-remove-open-link-handler`) | zero remaining references to `open_link` in any runtime/source file (verified across the whole tracked tree excluding `recovery-reports/`, which necessarily still names the finding and this removal — historical record, not code); 13/15 test scripts run directly, 0 failures | Scott (merge) |
