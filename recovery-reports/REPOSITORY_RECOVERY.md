...
| RR-09 | CI: adopt automated enforcement, or explicitly accept manual enforcement as permanent | §3 "Test situation" | code complete, PR open, awaiting Scott's merge — **Scott decided: require CI** (§7) | PR #15 (`recovery/rr-09-add-ci`) | Real, completed GitHub Actions runs on this PR (not simulated) — all 15 documented test scripts pass, including both meson-dependent ones with a genuine `meson install`: `test_installed_layout.py` 47/0, `test_desktop_integration.py` 37/0, all others clean. Two real CI-environment bugs found and fixed getting here: `apt-get` hanging on an interactive `needrestart` prompt, and WebKitGTK's `bwrap` sandbox failing under Ubuntu 24.04's default AppArmor unprivileged-userns restriction — both documented in the PR | Scott (decided CI; merge is the remaining step) |
